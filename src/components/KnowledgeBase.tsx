...
function ConversationCard({ conv, onSelect, onArchive, showArchive }: { conv: Conversation; onSelect: (id: string) => void; onArchive: (id: string) => void; showArchive: boolean }) {
  ...
  return (
    <div className="flex w-full items-start gap-3 rounded-2xl border border-border bg-card/60 p-4 text-left transition hover:border-border/70 hover:bg-secondary/30">
      ...
      {showArchive && (
        <button onClick={() => onArchive(conv.id)}
                className="flex shrink-0 items-center gap-1 rounded-lg border border-border bg-background px-2.5 py-1 text-[11px] text-muted-foreground transition hover:bg-secondary hover:text-foreground focus:outline-none focus:ring-1 focus:ring-primary/40">
    ...
    <div className="flex w-full items-start gap-3 rounded-2xl border border-border bg-card/60 p-4 text-left transition hover:border-border/70 hover:bg-secondary/30">
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <button onClick={onDelete}
                className="ml-1 flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground/50 transition hover:bg-destructive/15 hover:text-destructive focus:outline-none focus:ring-1 focus:ring-destructive/40"
